//
//  XJUICollectionViewLayout.swift
//  XJBaseFramework
//
...
open class XJUICollectionViewLayout: UICollectionViewLayout {
    ...
    private func prepareItemAttributes() {
        ...
        for section in 0..<collectionView.numberOfSections {
            ...
            // sectionHeader
            let headerSize = self.headerSize(atSection: section)
            ...
            headerAttributes.frame = CGRect(x: offset.x, y: offset.y, width: headerSize.width, height: headerSize.height)
            self.headerAttributes[section] = headerAttributes
            offset.y = offset.y + headerSize.height
            ...
            //sectionFooter
            let footerSize = self.footerSize(atSection: section)
            ...
            footerAttributes.frame = CGRect(x: sectionMargin.left + sectionPadding.left, y: offset.y, width: footerSize.width, height: footerSize.height)
            self.footerAttributes[section] = footerAttributes
            offset.y = offset.y + footerSize.height
    ...
    override open func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        + self.headerAttributes.values.filter { $0.frame.intersects(rect) }
        + self.footerAttributes.values.filter { $0.frame.intersects(rect) }
    }
...
extension XJUICollectionViewLayout {
    var delegate: XJUICollectionViewDelegateLayout? {
        return self.collectionView?.delegate as? XJUICollectionViewDelegateLayout
    }
    ...
    public func headerSize(atSection section: Int) -> CGSize {
        guard let collectionView = self.collectionView, let delegate = self.delegate else { return .zero }
        return delegate.collectionView?(collectionView, layout: self, headerSizeAt: section) ?? .zero
    }
    
    public func footerSize(atSection section: Int) -> CGSize {
        guard let collectionView = self.collectionView, let delegate = self.delegate else { return .zero }
        return delegate.collectionView?(collectionView, layout: self, footerSizeAt: section) ?? .zero
    }
}
